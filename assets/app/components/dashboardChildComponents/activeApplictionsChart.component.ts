...
@Component({
    selector:'dashboard_chart4',
    templateUrl:'./totalApplicationsChart.component.html'

})

export class ActiveApplictionsChartComponent implements AfterViewInit
{
...
status:string;
  ...
  constructor(private loginservice:OAOadminService,private router:Router){
    ...
    setTimeout(() => {
    console.log("FLag in totalappplication is ",DashboardComponent.flag);
    this.TotalApplications = DashboardComponent.flag_active;
   if(!(DashboardComponent.flag_active))
    {
    ...
    this.loginservice.botContactedFields(false, false, true,camp,startdate,endate)
      .subscribe(
      data => {
        var remind = [];
        var count = [];
    ...
    this.loginservice.NewExistingCustomer(false, false, true,camp,startdate,endate)
      .subscribe(
      data => {
        var custype = [];
        var count = [];
    ...
    setTimeout(()=>{
        this.loginservice.applicationByProduct(false,false,true,camp,startdate,endate)
          .subscribe(
          data => {
            console.log('sample');
            console.log(data);
    ...
    this.loginservice.ApplicationOnDeviceType(false, false, true,camp,startdate,endate)
      .subscribe(
      data => {
        var device = [];
        var count = [];
