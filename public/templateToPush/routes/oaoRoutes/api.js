...
//var homeContent = require('../../public/contents/homeContent');
...
OAORouter.post('/upload', function (req, res, next) {
    upload(req, res, function (err) {
        // console.log(req);
        OAOApplicationHelper.cmisUpload(req,function(success){
            if(success==true){
...
.post(function(req, res) {
    console.log("Applicants called...");
    console.log("Admins is"+req.body.adminId);
    var completed_by;
        ...
        OAODBHelper.GenerateApplicationReferenceId(req, res, function (result) {
            var app_id_ = req.body.application_id || OAOApplicationHelper.RefIdFormater(Number(result[0].app_ref_id) + 1, req);
	    ...
	    result.campaign_id=req.body.campaign_id,
            ...
            if (req.body.postal_home_address_flag == true) {
                ...
                if (req.body.sec_2_v == false) {
                    ...
                    if (req.body.employed != null && req.body.skip == false && req.body.asset_liability == false) {
                    ...
                    } else if (req.body.skip == false && req.body.asset_liability == true) {
...
.post(function(req, res) {
    OAODBHelper.GenerateApplicationReferenceId(req, res, function(result) {
            console.log("PersonalLoanApplicants...");
            console.log(req.body);
            var app_id_ = req.body.application_id || OAOApplicationHelper.RefIdFormater(Number(result[0].app_ref_id) + 1,req);
        var Oao_product_customer_details = new OAOApplicantSchema({
            product_code: req.body.product_code,
   ...
   OAODBHelper.getSubProductByParent(subproduct_id,child_of,function(result){
    ...
    OAORouter.route('/CrossSellApplicants').post(function (req, res) {
      ...
      OAODBHelper.GenerateApplicationReferenceId(req, res, function (result) {
        ...
        var app_id_ =  OAOApplicationHelper.RefIdFormater(Number(result[0].app_ref_id) + 1,req);
...
.get(function(req,res){
        /*var file = './public/contents/homeContent.json'
         homeContent.obj = {name: 'JP'}
 
        jsonfile.writeFile(file,JSON.stringify(homeContent), function (err) {
  console.error(err)
})*/
var output=null
...
.on('finish', function(response) {
    console.log("in response");
     //var x = require('../../public/contents/Product1');
